  ...
  <div class="wrap">
      ...
      <div class="container">
        ...
        <div class="course-list">
          <ul>
            ...
            <li class="sep"></li>
            <li class=detail-item>学习人数：{{courseDetailTop.learn_number}}</li>
            <li class="sep"></li>
    ...
    <div class="course-review">
      <ul class="review-head-wrap">
        ...
        <li class="head-item" @click="questionHandler()">
          常见问题
          <!--<Question></Question>-->
        </li>
    ...
    <div class="course-price">
      <div class="container">
        ...
        <ul class="course-price-item">
          <li v-for="item in pricePolicy" @click="choicePrice(item.id)"
              :class="{active:currentIndex===item.id}">
            <p class="price" :class="{active:currentIndex===item.id}">{{item.price}}</p>
            <p class="time" :class="{active:currentIndex===item.id}">{{item.time}}</p>
          </li>
        </ul>
        <div class="course-action">
          <button class="left" @click="addShoppingCar()">购买</button>
          <button class="right" @click="addShoppingCar()">加入购物车</button>
        </div>
      </div>
    </div>
...
<script>
  export default {
    ...
    data() {
      return {
        courseDetailTop: {},    // 课程顶部数据，详情数据使用{}，不需要循环后渲染，多个数据使用[]
        currentIndex: 0,
        pricePolicy: [
          {id: 1, price: 9.9, time: "有效期1个月"},
          {id: 2, price: 19.9, time: "有效期3个月"},
          {id: 3, price: 99, time: "有效期6个月"},
        ],
        valid_period: 0
      }
    },
    methods: {
      getCourseDetailTop() {   // 获取课程详细顶部数据
        this.$api.courseDetailTop(this.$route.params.detailId)
          .then(res => {
            if (!res.error_no) {
      ...
      getCoursePayment() { // 获取课程详细数据
        this.$api.coursePayment(this.$route.params.detailId)
          .then(res=>{
            if (!res.error_no){
              this.valid_period = res.data.valid_period
            }
          }).catch(err=>{
            console.log(err)
        })
      ...
      addShoppingCar() {     // 加入购物车
        if (this.currentIndex) {
          if (window.localStorage.getItem("access_token")) {    // token存在
            // 用户购买课程
            let course = {
              courseId: this.$route.params.detailId,
              validPeriodId: this.valid_period,
            };
            this.$api.shoppingCart(course).then(res=>{
              if (res.error_no === 0){  // 购物车中没有此课程，显示添加成功
                this.$message(res.data.status)
              }
              if (res.error_no === 10){ // 购物车中有此课程，显示更新成功
